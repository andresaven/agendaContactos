...
import controlador.ControladorAdd;
...
public class Add extends JDialog{
	...
	public Add(JFrame vistaPrincipal) {
	...
	private void varInit () {
		...
		add = new JLabel();
		ImageIcon contactIcon = new ImageIcon("img/contacto.png");
		add.setIcon(contactIcon);
		add.setBounds(163, 27, 48, 48);
		add.setBackground(Color.WHITE);
		getContentPane().add(add);
		
		lAdd = new JLabel("Añadir contacto");
	...
	public void establecerListeners(ControladorAdd controladorAdd) {
		volver.addActionListener(controladorAdd);
		bAdd.addActionListener(controladorAdd);
